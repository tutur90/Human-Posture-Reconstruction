...
data_path=r'/data/shuoyang67/MPI-INF-3DHP/mpi_inf_3dhp_test_set'
cam_set = [0, 1, 2, 4, 5, 6, 7, 8]
# joint_set = [8, 6, 15, 16, 17, 10, 11, 12, 24, 25, 26, 19, 20, 21, 5, 4, 7]
joint_set = [0, 1 , 5, 6, 7, 2, 3, 4, 11, 12, 13, 8, 9, 10, 14, 15]
...
for root, dirs, files in os.walk(data_path):
    ...
    for file in files:
        if file.endswith("mat"):
            ...
            data_2d = np.squeeze(data['annot2'][:])[:, :16]
            data_3d = np.squeeze(data['univ_annot3'][:])[:, :16]

            data_2d = np.float32(data_2d)
            data_3d = np.float32(data_3d)
...
np.savez_compressed('data_test_univ_3dhp', data=dic_seq)
