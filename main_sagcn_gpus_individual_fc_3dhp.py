...
from common.s_agcn import S_AGCN as Model
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(dataLoader):
        ...
        out_target[:, :, 14] = 0
        gt_3D = gt_3D.reshape(N, -1, opt.out_joints, opt.out_channels).float().to(device)
        ...
        if opt.test_augmentation and split == 'test':
            ...
            input_2D = input_2D.reshape(N, -1, opt.n_joints, opt.in_channels,
                                     1).permute(0, 3, 1, 2, 4).float().to(device)
...
if __name__ == '__main__':
    ...
    if opt.reload:
        ...
        pre_dict = torch.load(no_refine_path, map_location=device)#torch.cuda.current_device())
    ...
    for epoch in range(1, opt.nepoch):
        ...
        if opt.test:
            #import pdb; pdb.set_trace()
            mpjpe = val(
                opt, actions, test_dataloader, model)
            mpjpe = mpjpe#[0]
